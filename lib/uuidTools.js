...
const ENCRYPT_MATRIX = randomInvertibleMatrix(122, randomBytesSeed('42'));
...
function randomInvertibleMatrix(size, randomGenerator) {
  ...
  while (true) {
    matrix = Array(size)
      .fill(0)
      .map((value) => {
        return bytesToBigInt(randomGenerator(16)) & BIT_MASK_FULL;
      })
...
export function findNextIndex(query, startIndex, direction) {
  ...
  if (query.length > UUID_LENGTH) return undefined;

  const padLength = UUID_LENGTH - query.length;
